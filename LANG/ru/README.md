...
- **📐 Математическая точность**: Две модели масштабирования (Fixed и Dynamic) для разных потребностей дизайна
...
| Модель | Философия | Идеальный случай использования | Поддерживаемые платформы |
|-------|------------|----------------|-------------------|
| **Fixed (FX)** | Логарифмическое масштабирование (утонченное) | Кнопки, отступы, поля, иконки | Android + iOS |
| **Dynamic (DY)** | Пропорциональное масштабирование (агрессивное) | Контейнеры, сетки, плавные шрифты | Android + iOS |
| **SDP / SSP** | Предварительно рассчитанные ресурсы | Прямое использование `@dimen` в XML | Android |
| **Физические единицы** | mm/cm/inch → Dp/Sp/Px/Points | Носимые устройства, печать, точные макеты | Android + iOS |
