...
**Full Android ↔ synonym table (constants, formulae refs):** [IMPLEMENTATION_ALIGNMENT_APPDIMENS_DYNAMIC.md](IMPLEMENTATION_ALIGNMENT_APPDIMENS_DYNAMIC.md). **Migrating 1.x/2.x names → 3.x packages:** [NAMING_AND_MIGRATION_1X_2X_TO_3X.md](NAMING_AND_MIGRATION_1X_2X_TO_3X.md) (**FIXED/DEFAULT→`scaled`**, **DYNAMIC→`percent`**, **BALANCED→`auto`**).
...
| **PERCENTAGE / legacy DYNAMIC (axis-fraction growth)** | `compose.percent` (`psdp`, …)—follow submodule README. | `AppDimens.shared.percentage(_).toPoints()` | `percentage(_)` | Dynamic builders / `.dy` ([appdimens_dynamic.dart](../appdimens-flutter/lib/src/appdimens_dynamic.dart)) | `percentage(_)` |
