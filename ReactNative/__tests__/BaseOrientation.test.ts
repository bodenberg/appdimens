 ...
 * Date: 2025-01-31
 *
 * Library: AppDimens React Native - Base Orientation Tests
 *
...
import {resolveScreenType, wouldInvert} from '../src/utils/orientationResolver';

describe('BaseOrientation', () => {
  describe('resolveScreenType', () => {
    it('should not invert when orientation is AUTO', () => {
      const result = resolveScreenType('lowest', 'auto', {
        width: 360,
        height: 800,
      });
    ...
    it('should not invert portrait design in portrait orientation', () => {
      const result = resolveScreenType('lowest', 'portrait', {
        width: 360,
        height: 800,
      });
      expect(result).toBe('lowest');
    ...
    it('should invert portrait design in landscape orientation', () => {
      const resultLowest = resolveScreenType('lowest', 'portrait', {
        width: 800,
        height: 360,
      });
      expect(resultLowest).toBe('highest');

      const resultHighest = resolveScreenType('highest', 'portrait', {
        width: 800,
        height: 360,
      });
    ...
    it('should not invert landscape design in landscape orientation', () => {
      const result = resolveScreenType('highest', 'landscape', {
        width: 800,
        height: 360,
      });
    ...
    it('should invert landscape design in portrait orientation', () => {
      const result = resolveScreenType('lowest', 'landscape', {
        width: 360,
        height: 800,
      });
      expect(result).toBe('highest');
  ...
  describe('Real-world scenarios', () => {
    it('should maintain card proportion when rotated', () => {
      // Card designed for portrait at 300dp width
      const portraitType = resolveScreenType('lowest', 'portrait', {
        width: 360,
        height: 800,
      });
      const landscapeType = resolveScreenType('lowest', 'portrait', {
        width: 800,
        height: 360,
      });
